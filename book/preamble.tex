...
% 符号兜底字体。Windows 上 Arial Unicode MS / Noto Sans CJK SC 都没有，此时若直接
% 落到 DejaVu Sans，带圈字母 ⓐ (U+24D0) 会缺字（DejaVu Sans 不含 Enclosed
% Alphanumerics；Noto Sans SC 含 ⓐ 却缺 ✗ U+2717 和 ₀ U+2080）。Segoe UI Symbol
% 是 Windows 自带且同时覆盖这一组符号的字体，优先于 DejaVu Sans。
...
% Menlo 是 macOS 自带字体；Windows 上没有，按 DejaVu Sans Mono → Consolas 兜底。
% （DejaVu 随 TeX Live 一起装，Windows 上通常已在系统字体目录里。）
...
% 同理不用可变字体 Noto Sans SC（见下方正文中文字体的说明）
\IfFontExistsTF{Heiti SC}{\setCJKfamilyfont{capcjksf}[AutoFakeBold=2.5]{Heiti SC}}{\IfFontExistsTF{Noto Sans CJK SC}{\setCJKfamilyfont{capcjksf}[AutoFakeBold=2.5]{Noto Sans CJK SC}}{\IfFontExistsTF{SimHei}{\setCJKfamilyfont{capcjksf}[AutoFakeBold=2.5]{SimHei}}{\setCJKfamilyfont{capcjksf}[AutoFakeBold=2.5]{DejaVu Sans}}}}
...
% Windows 用系统自带的 SimSun / SimHei（宋体 / 黑体）。
%
% ⚠️ 别把 Windows 11 自带的 "Noto Sans SC" / "Noto Serif SC" 加进来：它们是**可变字体**
%    （NotoSansSC-VF.ttf / NotoSerifSC-VF.ttf），xdvipdfmx 处理不了，会在写 PDF 阶段直接
%    `xdvipdfmx:fatal: Invalid font: -1 (0)`，而 TeX 侧一行报错都没有，极难排查。
%    所以这里选静态字体。（另注意 "Noto Sans SC" 与 CI 上的 "Noto Sans CJK SC" 是两个
%    不同的字体族：前者 Google Fonts 版，后者 Source Han 版。）
...
\IfFontExistsTF{Songti SC}{%
  ...
  % TeX Live 自带的 Fandol，最后的兜底
  \setCJKmainfont[AutoFakeBold=2.5]{FandolSong}%
  \setCJKsansfont[AutoFakeBold=2.5]{FandolHei}%
  \setCJKmonofont[AutoFakeBold=2.5]{FandolSong}%
}}}
